...
public class AccesoDatos {
	...
	public static int comprobarPedido_articulo(Long numero_pedido, Long codigo_articulo) {
		int resultado = 0;
		Statement smt;
		String sentencia = "SELECT numero_pedido, codigo_articulo FROM pedido_articulo WHERE numero_pedido = '"
	...
	public static void eliminarPedido(Long numero_pedido) {
		Statement smt;
		...
		try {
			smt = conn.createStatement();
			smt.executeQuery(sentencia_pedido);
			smt.executeQuery(sentencia_pedidoArticulos);
		} catch (SQLException e) {
			System.out.println("Error obteniendo datos");
			e.printStackTrace();
		}
		...
		try {
			...
			while (rs.next()) {
				System.out.println("\tNumero pedido: " + rs.getLong("numero_pedido") + "\n\t"
						+ rs.getLong("numero_cliente") + "\n\t" + rs.getString("fecha")); // Imprime el resultado de la consulta
			}
		} catch (SQLException e) {
			System.out.println("Error obteniendo datos");
			e.printStackTrace();
	...
	public static void mostrarPedido_articulo(Long numero_pedido) {
		...
		try {
			...
			while (rs.next()) {
				System.out.println("\tNumero pedido: " + rs.getLong("numero_pedido") + "\n\t"
						+ rs.getLong("codigo_articulo") + "\n\t" + rs.getInt("cantidad")); // Imprime el resultado de la consulta
			}
		} catch (SQLException e) {
			System.out.println("Error obteniendo datos");
			e.printStackTrace();
	...
	/*
	 * public static void borrarDatosTabla() { PreparedStatement smt; String
	 * sentencia = "DELETE FROM diccionario"; try { smt =
	 * conn.prepareStatement(sentencia); smt.executeUpdate(); } catch (SQLException
	 * e) { System.out.println("Error borrar palabra"); } }// fin_borrarDatosTabla
	 * 
	 * public static TreeMap<String, Integer> listarDiccionario() { Statement smt;
	 * TreeMap<String, Integer> mapaDiccionario = new TreeMap<String, Integer>();
	 * String sentencia = "SELECT * FROM diccionario"; try { smt =
	 * conn.createStatement(); ResultSet rs = smt.executeQuery(sentencia); while
	 * (rs.next()) { mapaDiccionario.put(rs.getString("Palabra"),
	 * rs.getInt("Frecuencia")); }
	 * 
	 * } catch (SQLException e) { System.out.println("Error obteniendo datos"); }
	 * return mapaDiccionario; }// fin_listarDiccionario
	 * 
	 * public static void modificarDiccionario(String palabra, Integer frecuencia) {
	 * Statement smt; String sentencia; try { smt = (Statement)
	 * conn.createStatement(); sentencia = "UPDATE diccionario SET frecuencia= " +
	 * frecuencia + " WHERE palabra = '" + palabra + "'"; smt.execute(sentencia); }
	 * catch (SQLException e) { System.out.println("Error actualizando"); } }//
	 * fin_modificarDiccionario
	 * 
	 * public static void borrarPalabra(String palabra) { PreparedStatement smt;
	 * String sentencia = "DELETE FROM diccionario WHERE palabra=?"; try { smt =
	 * conn.prepareStatement(sentencia); smt.setString(1, palabra);
	 * smt.executeUpdate(); } catch (SQLException e) {
	 * System.out.println("Error borrar palabra"); } }// fin_borrarConctacto
	 */
}
