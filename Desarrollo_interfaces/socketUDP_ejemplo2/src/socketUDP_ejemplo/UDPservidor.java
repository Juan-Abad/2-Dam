...
public class UDPservidor {
	...
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			...
			try {
				...
				case "registro":
					if (clientesConectados.size() < 2 && clientesConectados.iterator().next().getAddress() != recibido.getAddress()) {
	...
	public static void empezarJuego(DatagramSocket socket) {
			...
			while (true) {
				...
				byte buf[] = mensajeRespuestaJson.getBytes();
				DatagramPacket packet = new DatagramPacket(buf, buf.length, clientesConectados.get(jugador_n).getAddress(), 12345);
				try {
					socket.send(packet);
					do {
						socket.receive(recibido);
					}while(recibido.getAddress() != clientesConectados.get(jugador_n).getAddress() && gson.fromJson(new String(recibido.getData(), 0, recibido.getLength()), JsonObject.class).get("tipo").getAsString() != "Jugada");
					int fila = gson.fromJson(new String(recibido.getData(), 0, recibido.getLength()), JsonObject.class).get("fila").getAsInt();
					int columna = gson.fromJson(new String(recibido.getData(), 0, recibido.getLength()), JsonObject.class).get("columna").getAsInt();
					System.out.println("fila: "+fila+", columna: "+columna);
					
					// Validar la jugada
					if (fila < 0 || fila > 2 || columna < 0 || columna > 2 || game.getTablero()[fila][columna] != "") {
						continue;
					}
