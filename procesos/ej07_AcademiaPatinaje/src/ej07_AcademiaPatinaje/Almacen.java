...
public class Almacen {
	private TreeMap<Integer, ArrayList<Patin>> mapaPatines;// mapa que guarda como clave el numero de pie de los
															// patines, y en el arraylist, se guardan pares de patines

	public Almacen() {
		mapaPatines = new TreeMap<>();
		for (int i = 34; i < 44; i++) {// for que crea los patines segun el numero de pie
			ArrayList<Patin> listaParesPatin = new ArrayList<>();// se crea un arraylist que guarda dos patines de ese
																	// numero de pie, cada vez
			...
			mapaPatines.put(i, listaParesPatin);// por cada numero de pie se guardan 2 pares de patines, se indica el
												// numero como clave y el arraylist con los dos pares
		}
	}

	synchronized public void almacenarPatin(Integer numeroPie, Boolean pieDerecho) {// metodo synchronized que almacena
																					// patines en el almacen
		if (mapaPatines.containsKey(numeroPie)) {// comprobamos que el mapa contenga el numero de pie
			for (Patin patin : mapaPatines.get(numeroPie)) {// recorremos el arraylist del mapa de ese numero de pie
				if (!patin.isParCompleto()) {// comprobamos si el par es completo, entra si no lo esta
					if (pieDerecho != null) {// si pie derecho no es null significa que quiere almacenar un solo pie de
												// patin izq o derecho
						if (pieDerecho && !patin.getPatinDerecho()) {// comprobamos que se quiere almacenar un pie
																		// derecho o no, y si es este el par de patines
																		// que le falta un patin derecho
							patin.setPatinDerecho(true);
						} else if (!patin.getPatinIzquierdo()) {// quiere almacenar un patin izq, comprueba si este es
																// el par que le falta un patin izq
							patin.setPatinIzquierdo(true);
						}
					} else {
						if (!patin.getPatinDerecho() && !patin.getPatinIzquierdo()) {// se quiere almacenar un par
																						// completo, pie izq y derecho,
																						// comprueba si este es el par
																						// que le faltan los dos
							patin.setPatinDerecho(true);
							patin.setPatinIzquierdo(true);
						}
					}
	...
	synchronized public void retirarPatin(Integer numeroPie, Boolean pieDerecho) {// metodo para retirar patines
		if (mapaPatines.containsKey(numeroPie)) {// comprueba que el pie que se recibe para retirar, existe en el
													// almacen
			for (Patin patin : mapaPatines.get(numeroPie)) {// recorre el arraylist del mapa para ese pie(clave del
															// mapa), cada iteracion es un patin
